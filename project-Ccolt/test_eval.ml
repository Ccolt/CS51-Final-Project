...
let exp5 = Let("x", Num(2), Let("y", Num(1), Binop(Plus, 
                                                   Var("x"), 
                                                   Var("y")))) ;;
let exp6 = Conditional(Bool(true), Num(1), Num(2)) ;;
let exp7 = App(Fun("x", Num(1)), Num(3)) ;;
let exp8 = Let("y", Num(2), (App (Fun("x", 
                                      Binop(Equals, Var("y"), 
                                      Var("x"))), 
                                      Num(1)))) ;;
let exp9 = Let("f", Fun("x", 
                        Binop(Plus, Var("x"), 
                        Num(1))), App(Var("f"), App(Var("f"), Num(3))))  ;;
let exp10 = Letrec("f", 
